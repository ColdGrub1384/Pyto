...
def __clear_mods__():
    ...
    for key in keys:
        try:
            mod = sys.modules[key]
            if os.access(mod.__file__, os.W_OK):
                del sys.modules[key]
            ...
            try:
                ...
                if not __isMainApp__() or replMode:
                    ...
                    if "cv2.error" in error and "!_src.empty()" in error:
                        ...
                        try:
                            PyOutputHelper.printError(string, script=threading.current_thread().script_path)
                        except AttributeError:
...
def input(prompt=None):
    ...
    while __PyInputHelper__.getUserInput(path) is None or threading.currentThread() in ignoredThreads:
        time.sleep(0.2)
