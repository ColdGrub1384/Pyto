...
def __runREPL__():
    ...
    clear.__repr__ = "Type 'clear()' to clear the console"
    
    __namespace__ = {"clear":clear}
    
    def read(prompt):
        import console
        console.__repl_namespace__ = __namespace__
    ...
    interact(readfunc=read, local=__namespace__)
                ...
                if not __isMainApp__() or replMode:
                    ...
                    try:
                        PyOutputHelper.printError(
                            error, script=threading.current_thread().script_path
                        )
                    except AttributeError:
