...
if False:
    ...
    BUNDLED_MODULES = ['certifi', 'chardet', 'colorama', 'cycler', 'dateutil', 'distlib', 'idna', 'ipaddress', 'jedi', 'lockfile', 'matplotlib', 'numpy', 'pandas', 'setuptools', 'progress', 'pyparsing', 'pytoml', 'pytz', 'queue', 'requests', 'rubicon', 'six', 'stopit', 'urllib3', 'webencodings', 'wsgiref', 'lxml']
    SITE_PACKAGES_FOLDER = ""
    if __platform__ is iOS:
        SITE_PACKAGES_FOLDER = os.path.expanduser("~/Documents/modules")
    elif __platform__ is macOS:
        SITE_PACKAGES_FOLDER = os.path.expanduser("~/.pyto/modules")
...
class ArchiveFileInstaller(object):
    ...
    def _unzip(self, pkg_name, archive_filename):
        ...
        if '.zip' in archive_filename:
            ...
            os.mkdir(d)
            os.system('tar -C {} -xzf {}'.format(d, archive_filename))
        elif '.bz2' in archive_filename:
            os.system('tar -C {} -jxf {}'.format(extracted_folder, archive_filename))
        else:  # gzip
            os.system('tar -C {} -zxf {}'.format(extracted_folder, archive_filename))
        ...
        for p in sorted(packages):  # folders or files under source root
            ...
            if p == '':  # no packages just files
                ...
                for f in ArchiveFileInstaller._find_package_files(from_folder):
                    ...
                    if use_2to3:
                        os.system('2to3 -w {} > /dev/null'.format(target_file))
                ...
                if use_2to3:
                    os.system("""find {} --name '.py' | xargs -n 1 -I %% 2to3 -w %% > /dev/null""".format(target_dir))
            ...
            if files_installed:
                for f in files_installed:
                    ...
                    if not os.path.isfile(f) and not os.path.isdir(f):
                        f = os.path.expanduser('~/Documents/modules/{}'.format(basename(f)))
...
class PyPIRepository(PackageRepository):
    ...
    def download(self, pkg_name, ver_spec, wheel_priority=False):
        ...
        worker = wget.main([source['url'], '-o', os.getenv('TMPDIR')+'/'+source['filename']])
        #worker = os.system('curl {} -o {}'.format(source['url'], source['filename']))
...
class GitHubRepository(PackageRepository):
    ...
    def download(self, owner_repo, ver_spec):
        ...
        #os.system('curl https://github.com/{0}/{1}/archive/{2}.zip -o $TMPDIR/{2}.zip'.format(
        #owner, repo, release
        #))
...
class UrlRepository(PackageRepository):
    ...
    def download(self, url, ver_spec):
        ...
        #os.system('curl {} -o $TMPDIR/{}'.format(url, archive_filename))
