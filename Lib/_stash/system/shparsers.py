        ...
        for ipseq in pseq_indices:
            ...
            for isc in range(0, len(pseq), 2):
                ...
                if (
                    ...
                    and simple_command.io_redirect.filename == ""
                ):
            ...
            if is_cmd_word:
                ...
                if word_to_complete != "":
                    script_names = [
                        ...
                        if name.startswith(word_to_complete)
                    ]
            else:
                script_names = []
    ...
    def path_match(self, word_to_complete):
        ...
        if os.path.isdir(full_path) and full_path.endswith("/"):
            for fname in os.listdir(full_path):
                ...
                path_names.append(
                    os.path.join(
                        os.path.dirname(word_to_complete), fname.replace(" ", "\\ ")
                    )
                )
            ...
            if os.path.isdir(d):
                for fname in os.listdir(d):
                    if fname.startswith(f):
                        ...
                        path_names.append(
                            os.path.join(
                                os.path.dirname(word_to_complete),
                                fname.replace(" ", "\\ "),
                            )
                        )
