...
def _end_of_encoding(encoding, start):
    ...
    while i < len(encoding):
        ...
        if c in b"([{<":
            ...
            raise ValueError(
                "Unknown encoding {} at index {}: {}".format(c, i, encoding)
            )

    if paren_depth > 0:
        raise ValueError(
            "Incomplete encoding, missing {} closing parentheses: {}".format(
                paren_depth, encoding
            )
        )
    else:
        raise ValueError(
            "Incomplete encoding, reached end of string too early: {}".format(encoding)
        )
...
def _ctype_for_unknown_encoding(encoding):
    if encoding.startswith(b"^"):
        ...
        raise ValueError(
            "A bit field encoding cannot appear outside a structure: %s" % (encoding,)
        )
    elif encoding.startswith(b"?"):
        raise ValueError(
            "An unknown encoding cannot appear outside of a pointer: %s" % (encoding,)
        )
    elif encoding.startswith(b"T") or encoding.startswith(b"t"):
        raise ValueError(
            "128-bit integers are not supported by ctypes: %s" % (encoding,)
        )
    elif encoding.startswith(b"j"):
        raise ValueError(
            "Complex numbers are not supported by ctypes: %s" % (encoding,)
        )
